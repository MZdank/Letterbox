 ...
 * 
 * code added by: Mark Zdankiewicz. MZdank@bu.edu
 */
...
public class LetterSquare {
    ...
    private boolean isValid(String letter, int wordNum, int charNum) {
        if (alreadyUsed(this.words[wordNum] + letter)){
            return false;
        }        
        if (charNum >= 1){
            if(onSameSide(letter, lastLetter(this.words[wordNum])) == true){
            return false;
            }
        }
        if (dictionary.hasString(this.words[wordNum] + letter) != true){
            return false;
        }
    ...
    for (int i = 0; i < this.letters.length; i++) {
        if (isValid(this.letters[i], wordNum, charNum)) {
            ...
            if (charNum >= 2 && dictionary.hasFullWord(this.words[wordNum])) {
    ...
    public static void main(String[] args) {
        ...
        String[] prompts = {"(Write characters without spaces) top side: ", "left side: ", 
